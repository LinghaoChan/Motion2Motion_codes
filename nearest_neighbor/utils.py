...
def extract_patches(x, patch_size, stride, loop=False):
    ...
    x_patches = (
        unfoldNd.unfoldNd(x, kernel_size=patch_size, stride=stride)
        .transpose(1, 2)
        .reshape(b, -1, c, patch_size)
    )
...
def combine_patches(x_shape, ys, patch_size, stride, loop=False):
    ...
    combined = unfoldNd.foldNd(
        ys.permute(0, 2, 1),
        output_size=tuple(out_shape[-1:]),
    ...
    if loop:
        ...
        combined[:, :, -2 * half : -half] = (
            front + combined[:, :, -2 * half : -half]
        ) / 2
