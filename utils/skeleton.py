...
class SkeletonPoolJoint(nn.Module):
    def __init__(self, topology, pooling_mode, channels_per_joint, last_pool=False):
        ...
        for x in range(len(self.parent)):
            if children_cnt[x] == 0 or (
                children_cnt[x] == 1 and children_cnt[self.child[x]] > 1
            ):
        ...
        self.weight = torch.zeros(
            len(self.pooling_list) * channels_per_joint,
            self.joint_num * channels_per_joint,
        )
        ...
        for i, pair in enumerate(self.pooling_list):
            for j in pair:
                for c in range(channels_per_joint):
                    self.weight[
                        i * channels_per_joint + c, j * channels_per_joint + c
                    ] = 1.0 / len(pair)
...
class SkeletonUnpool(nn.Module):
    def __init__(self, pooling_list, channels_per_edge):
        ...
        self.description = "SkeletonUnpool(in_joint_num={}, out_joint_num={})".format(
            self.input_joint_num,
            self.output_joint_num,
        )

        self.weight = torch.zeros(
            self.output_joint_num * channels_per_edge,
            self.input_joint_num * channels_per_edge,
        )
