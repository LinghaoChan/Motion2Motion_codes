        ...
        for i, track in enumerate(self.tracks_json):
            ...
            if i == 0:
                ...
                if rotation.shape[0] == 0:
                    ...
                    rotation = np.repeat(
                        rotation, self.num_frames // rotation.shape[0], axis=0
                    )
                elif rotation.shape[0] > self.num_frames:
...
class TracksMotion:
    ...
    def parse(
        ...
        for i, _track in enumerate(out_tracks_json):
            if i == 0:
                times = [
                    j * out_tracks_json[i]["times"][1] for j in range(motion.shape[0])
                ]
                out_tracks_json[i]["values"] = (
                    pos.flatten().detach().cpu().numpy().tolist()
                )
            else:
                out_tracks_json[i]["values"] = (
                    rot[:, i - 1, :].flatten().detach().cpu().numpy().tolist()
                )
