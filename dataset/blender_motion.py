...
class BlenderMotion:
    def __init__(
        ...
        def to_tensor(motion_data, repr="euler", rot_only=False):
            ...
            if (
                repr == "quaternion" or repr == "quat" or repr == "repr6d"
            ):  # default is euler for blender data
                rotations = torch.tensor(motion_data[:, 3:], dtype=torch.float).view(
                    motion_data.shape[0], -1, 3
                )
            if repr == "quat":
        ...
        self.motion_data = MotionData(
            to_tensor(motion_data, repr=repr).permute(1, 0).unsqueeze(0),
            repr=repr,
            use_velo=use_velo,
